...
<div class="container-fluid">
    ...
    <!-- Page Heading -->
    <h1 class="h4 mb-3 text-gray-800">Tambah Transaksi Pengembalian</h1>
        ...
        <div class="col-lg-12">
            ...
            <div class="card border-primary">
                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                    <h6 class="m-0 font-weight-bold text-dark card-title"><i class="fas fa-fw fa-book"></i> Daftar Buku Yang Dipinjam</h6>
                ...
                <div class="card-body p-3">
                    <div class="row mb-2">
                        <div class="col-md-4">
                            <form class="navbar-search" action="index.php?include=tambah-pengembalian" method="post">
                        ...
                        <tbody>
                            <?php
                                ...
                                if (!empty($katakunci_kode)) {
                                    $sql .= " AND `peminjaman`.`kode_peminjaman` LIKE '%$katakunci_kode%'";
                                } else if(!empty($katakunci_nama)) {
                                    $sql .= " AND `users`.`nama` LIKE '%$katakunci_nama%'";
                                } else if(!empty($katakunci)) {
                                    $sql .= " AND `users`.`nama` LIKE '%$katakunci%'";
                                }

                                $sql .= " GROUP BY `peminjaman`.`kode_peminjaman` ORDER BY `peminjaman`.`tgl_kembali` LIMIT $posisi, $batas";
                            ...
                            <tr>
                                ...
                                <td class="align-middle text-center">
                                    <a href="index.php?include=proses-pengembalian&data=<?= $kode_peminjaman; ?>" class="badge badge-success"><i class="fas fa-fw fa fa-undo"></i> Proses Pengembalian</a>
                                </td>
                            </tr>
                            <?php $no++; ?>
                            <?php endwhile; ?>
                    ...
                    <?php
                        ...
                        // jika kata kunci tidak kosong
                        if (!empty($katakunci_kode)) {
                            $sql_jum .= " AND `peminjaman`.`kode_peminjaman` LIKE '%$katakunci_kode%'";
                        } else if(!empty($katakunci_nama)) {
                            $sql_jum .= " AND `users`.`nama` LIKE '%$katakunci_nama%'";
                        } else if(!empty($katakunci)) {
                            $sql_jum .= " AND `users`.`nama` LIKE '%$katakunci%'";
                        }
                        
                        $sql_jum .= " GROUP BY `peminjaman`.`kode_peminjaman` ORDER BY `peminjaman`.`tgl_kembali`";
                    ...
                    <nav aria-label="Page navigation example">
                        ...
                        <?php 
                            if ($jum_halaman == 0) {
                                ...
                                if ($halaman != 1) {
                                echo "<li class='page-item'><a class='page-link' href='index.php?include=tambah-pengembalian&halaman=1'>First</a></li>";
                                echo "<li class='page-item'><a class='page-link' href='index.php?include=tambah-pengembalian&halaman=$sebelum'>&laquo;</a></li>";
                                }
                                for ($i = 1; $i <= $jum_halaman; $i++) {
                                    if ($i > $halaman - 5 and $i < $halaman + 5 ) {
                                        if ($i != $halaman) { 
                                            echo "<li class='page-item'><a class='page-link' href='index.php?include=tambah-pengembalian&halaman=$i'>$i</a></li>";
                                        } else {
                                ...
                                echo "<li class='page-item'><a class='page-link' href='index.php?include=tambah-pengembalian&halaman=$setelah'>»</a></li>";
                                echo "<li class='page-item'><a class='page-link' href='index.php?include=tambah-pengembalian&halaman=$jum_halaman'>Last</a></li>";
                                }          
                            }
                        ?>
                        </ul>
